...
SCRIPT = [
    (
        "AGENT",
        0.5,
        "Thank you for calling Sunrise Orthopedics. This call may be recorded. May I have your first and last name?",
    ),
    (
        "PATIENT",
        0.8,
        "Hi, sure. It's Maya Thompson. I'm calling because my knee's been hurting and I'd like to book a first appointment.",
    ),
    ...
    (
        "AGENT",
        1.0,
        "Bring a photo ID and your insurance card, and arrive fifteen minutes early. Is there anything else I can help with?",
    ),
...
def main() -> None:
    ...
    def channel(spk: str, out: Path) -> None:
        ins, filters, labels = [], [], []
        for k, (_spk, start, _dur, wav) in enumerate([x for x in items if x[0] == spk]):
            ins += ["-i", str(wav)]
    ...
    meta = {
        "stem": stem,
        "scenario_id": "01-schedule-new-patient",
        "target_id": "local-clinic",
        "title": "SYNTHETIC — pipeline smoke test",
        "call_id": "synthetic-call",
        "started_at": datetime.now(UTC).isoformat(),
        "ended_at": None,
        "ended_reason": "synthetic",
        "from": settings.caller_number or "local",
        "to": "local-clinic",
        "cost_usd": 0,
        "performance_metrics": None,
        "files": {
            "recording_mp3": f"recordings/{stem}.mp3",
            "transcript_md": None,
            "call_json": None,
            "events_jsonl": "reports/events/synthetic-call.jsonl",
        },
    }
