...
| 2 | **Audio arena (Pipecat)**: simulated caller ↔ bundled sample agent over real audio (VAD, turn-taking, barge-in) in-process/websocket; STT/TTS via Deepgram *or* fully local (faster-whisper / Kokoro); stereo recording from the pipeline | ⏳ next |
