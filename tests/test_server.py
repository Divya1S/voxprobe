...
pytestmark = pytest.mark.skipif(
    not (os.environ.get("GROQ_API_KEY") or os.path.exists(".env")), reason="needs LLM keys"
)
...
def _vapi_style_request(scenario_id: str) -> dict:
    return {
        ...
        "messages": [
            ...
            {"role": "user", "content": "Thanks for calling. This call may be recorded. May I have your first and last name?"},
        ],
    }
...
def test_rejects_bad_token_when_secret_set(client):
    ...
    r = c.post("/chat/completions", json=_vapi_style_request("01-schedule-new-patient"),
...
def test_streams_openai_chunks_with_a_short_spoken_reply(client):
    ...
    with c.stream("POST", "/chat/completions", json=_vapi_style_request("01-schedule-new-patient"),
